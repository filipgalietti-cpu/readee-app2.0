  ...
  createClient: async () => ({
    ...
    from: (table: string) => {
      const q = {
        select: () => q,
        eq: (...args: unknown[]) => {
          mock.filters.push([table, ...args]);
          return q;
        },
        order: () => q,
        limit: () => q,
        single: async () => ({
          data: { plan: "free", created_at: "2026-09-01", had_subscription: false },
          error: null,
        }),
        ...
        maybeSingle: async () =>
          table === "children"
            ...
            : {
                ...mock.saved,
                data: mock.saved.data ? { decision: {}, ...mock.saved.data } : null,
              },
      };
      return q;
    },
...
describe("the placement start unit is free only for its reader", () => {
  ...
  it("unlocks the assigned domain at the assessed grade without unlocking other units", () => {
    ...
    expect(isLessonInFreeUnit({ ...firstUnit, grade: "4th Grade" }, defaults, firstUnit)).toBe(
      false,
    );
    expect(isLessonInFreeUnit({ ...firstUnit, domain: "Language" }, defaults, firstUnit)).toBe(
      false,
    );
  ...
  it("routes a second-grade placement to its actual first lesson", async () => {
    await expect(StartPage({ searchParams: Promise.resolve({ child: childId }) })).rejects.toThrow(
      `redirect:/learn?child=${childId}&standard=RI.2.1`,
    );
  });
  it("serves that lesson to a free parent, but still gates other premium units", async () => {
    expect(
      await LearnPage({ searchParams: Promise.resolve({ child: childId, standard: "RI.2.1" }) }),
    ).toBeTruthy();
    await expect(
      LearnPage({ searchParams: Promise.resolve({ child: childId, standard: "L.2.1" }) }),
    ).rejects.toThrow(`redirect:/journey?child=${childId}`);
  });
  it("does not accept the child query parameter as proof of access", async () => {
    mock.child = { data: null, error: null };
    await expect(
      LearnPage({ searchParams: Promise.resolve({ child: childId, standard: "RI.2.1" }) }),
    ).rejects.toThrow("redirect:/placement/setup");
  });
});
