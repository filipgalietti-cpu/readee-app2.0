...
import { activeList, createLadder, needsFoundations, recordWord, type PlacedBand } from "@/lib/placement/ladder";
...
describe("assessment audit: enrollment to reading level to journey", () => {
  it("requires follow-ups instead of assuming one grade lower is comfortable", () => {
    ...
    sub.passages[0] = { band: 4, wordsCorrect: 20, wordsTotal: 100, durationSeconds: 60, minuteWordsCorrect: 20, minuteSeconds: 60 };
    ...
    for (const reading of [0, 1, 2, 3, 4] as const) {
      it(`enrolled ${enrolled}, reads at ${reading}: bank-valid evidence reaches the matching journey`, () => {
        ...
        while (!ladder.done) {
          ...
          ladder = recordWord(ladder, PLACEMENT_BANK.bands[list.band].words[list.attempts.length].word, list.band <= reading);
        }
        sub.ladder = ladder;
        sub.passages = reading === 0 ? [] : [reading].map((band) => ({
          band: band as PlacedBand, wordsCorrect: 70, wordsTotal: 72, durationSeconds: 60, minuteWordsCorrect: 70, minuteSeconds: 60,
        }));
        ...
        const journey = computeJourneyProgress({ readingLevel: decision.readingLevelName, placement: plan, practice: [], lessonProgress: [] });
...
describe("assessment audit: recognition timing", () => {
  ...
  it.each(["delayed speech", "silence", "stalled stop"])("handles passage capture: %s", async (mode) => {
    ...
    const box: Record<string, any> = {
      module: { exports: {} }, console, Date, Promise, setTimeout, clearTimeout,
    ...
    box.exports = box.module.exports; box.globalThis = box;
    const source = readFileSync("app/(protected)/placement/_components/PlacementRunner.tsx", "utf8").replace(
      "  // ───────────────────────────────────────────────── render",
      "  globalThis.readPassageOnce = readPassageOnce; return null;\n  // ───────────────────────────────────────────────── render",
    );
