...
vi.mock("@/lib/schemas", () => ({ PlacementSubmissionSchema: { safeParse: (body: unknown) => ({ success: true, data: body }) } }));
vi.mock("@/lib/supabase/server", () => ({ createClient: async () => ({ auth: { getUser: async () => { if(mock.throws) throw new Error("provider failure"); return { data: { user: mock.signedIn ? { id: "parent" } : null } }; } },
  from: () => { const q = { select: () => q, eq: () => q, maybeSingle: async () => ({ data: { parent_id: "parent", first_name: "Test", grade: "4th Grade" }, error: mock.childError }) }; return q; },
}) }));
vi.mock("@/lib/supabase/admin", () => ({ supabaseAdmin: () => ({ from: () => { const q = { insert: () => q, select: () => q, single: async () => ({ data: null, error: { code: "08006" } }) }; return q; } }) }));
vi.mock("@/lib/placement/decide", () => ({ decidePlacement: () => ({ gradeKey: "2nd", readingLevelName: "2nd Grade", relative: { label: "below" }, decoding: { level: 2 }, strengths: [], needs: [], seeds: [] }) }));
vi.mock("@/lib/placement/plan", () => ({ buildPlan: () => ({}) }));
vi.mock("@/lib/placement/narration", () => ({ narrate: () => [] }));
...
const request = () => new Request("http://localhost/api/placement/complete", { method: "POST", body: JSON.stringify({ childId: "child", enrolled: 4, moments: [], ladder: {}, durationSeconds: 10 }) });

describe("placement save reporting", () => {
  beforeEach(() => { vi.clearAllMocks(); mock.signedIn = true; mock.throws = false; mock.childError = null; });
  it("reports a failed insert and returns its correlation ID without scheduling emails", async () => {
