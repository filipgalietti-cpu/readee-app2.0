...
const LADDER: Record<string, { below: string; next: string; opts: string }> = {
  ...
  "4th Grade": { below: "Grade 3 support", next: "Grade 5", opts: "3 options for easier (a Grade 3 bridge, picture support only where the picture is real evidence), 4 TEXT options with plausible parallel-length distractors for core, at least one core item asking for the BEST evidence among four true details, two on-screen read-aloud items per quiz; harder = Grade 5 transfer whose new tool is TAUGHT in the question stimulus first (quoting accurately, two or more main ideas, comparing two accounts of one event, figurative language in depth), then applied" },
};
