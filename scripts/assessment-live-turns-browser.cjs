...
(async () => {
  ...
  try {
    ...
    await expect(page.getByRole('button',{name:'That’s my name. Let’s read',exact:true})).toBeVisible();
    await expect(page.locator('[data-name-luna]')).toBeVisible();
    await expect(page.getByText(/A grown-up can fix the spelling/)).toHaveCount(0);
