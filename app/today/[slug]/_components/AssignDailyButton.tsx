...
/**
 * Teacher-only button on /today/[slug] that converts the daily into
 * a custom_quiz + assignment in their selected classrooms.
 *
 * Renders nothing for anonymous visitors / parents — only for users
 * with role=educator who own at least one classroom. The component
 * fetches its own auth state on mount so the public `/today` page
 * can stay a static-friendly server component.
 */
export default function AssignDailyButton({ date }: { date: string }) {
  ...
  // Detect teacher + classroom ownership client-side.
  useEffect(() => {
    let cancelled = false;
  ...
  function submit() {
    ...
    start(async () => {
      const res = await assignDailyQuestion({
        date,
      ...
      setDoneMsg(
        `Assigned to ${res.assignedTo} classroom${res.assignedTo === 1 ? "" : "s"}. Open one to see it.`,
      );
      router.refresh();
    });
  }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <h3 className="text-lg font-bold text-zinc-900">
              Assign today's Readee Daily
            </h3>
            <p className="mt-0.5 text-sm text-zinc-500">
              Pick the classrooms that should get this passage as an
              assignment. Students will see it in their queue.
            </p>
