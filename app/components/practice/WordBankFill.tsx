...
export function WordBankFill({
  ...
  return (
    <div className="flex flex-col gap-6">
      <h2 className="text-[22px] font-bold text-zinc-900 leading-snug text-center">
        {prompt}
      </h2>
              ...
              return (
                <AnimatePresence mode="wait" key={idx}>
                  {selected ? (
                    <motion.span
                      key="filled"
                      className="inline-block px-3 py-1 rounded-lg bg-indigo-100 text-indigo-800 font-extrabold border-2 border-indigo-300"
        ...
        {wordBank.map((word, i) => {
          ...
          if (done) {
            ...
            if (isSelected && result === "correct") {
              style =
                "bg-emerald-200 text-emerald-900 border-emerald-500 ring-2 ring-emerald-400/40";
            } else if (isSelected && result === "incorrect") {
              style =
                "bg-red-200 text-red-900 border-red-500 ring-2 ring-red-400/40";
