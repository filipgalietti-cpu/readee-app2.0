...
export function SoundMachine({
  ...
  const handleTapBank = useCallback(
    (soundIdx: number) => {
      console.log("[SoundMachine:v3] bank tap idx=", soundIdx, "sound=", allSounds[soundIdx], "onPlayPhoneme_type=", typeof onPlayPhoneme);
      if (answered || result !== null) return;
      console.log("[SoundMachine:v3] calling onPlayPhoneme with:", allSounds[soundIdx]);
      try {
        onPlayPhoneme?.(allSounds[soundIdx]);
        console.log("[SoundMachine:v3] onPlayPhoneme returned OK");
      } catch (err) {
        console.error("[SoundMachine:v3] onPlayPhoneme THREW:", err);
      }
    ...
    <div className="flex flex-col gap-6">
      ...
      {imageUrl && (
        <div className="flex justify-center">
          <LoadingImage
            ...
            className="w-40 h-40 object-contain rounded-2xl border-2 border-white shadow-md bg-white/60"
      ...
      >
        ...
        <div className="flex gap-2 justify-center flex-wrap">
          {placed.map((soundIdx, slotIdx) => (
            ...
            >
              {soundIdx === -1 ? (
                <span className="text-zinc-400 text-sm">{slotIdx + 1}</span>
              ) : (
                <motion.span
      ...
      <AnimatePresence>
        {result === "correct" && (
          ...
          >
            <span className="inline-block text-4xl font-black text-emerald-600 bg-emerald-50 px-6 py-3 rounded-2xl border-2 border-emerald-300">
              {targetWord}
            </span>
          </motion.div>
        )}
