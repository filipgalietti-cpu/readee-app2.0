...
export const picturesTellMoreImages: Record<string, string | { subject: string; ref?: string }> = {
  ...
  "page-4": { subject: "The same young boy with curly black hair in a striped orange t-shirt grinning happily on the back porch at night, and down on the porch floorboards in front of him TWO raccoons sit side by side sharing blueberries from one wide teal dish: a big chubby raccoon with a black mask and ringed striped tail, and sitting right beside it on the floor a tiny fluffy baby raccoon half its size with the same black mask holding one blueberry in its little paws, both raccoons firmly seated on the wooden porch floor next to the dish, the night sky above completely empty except for one big full moon and small stars, fireflies glowing over the grass", ref: "page-3" },
  ...
  "quiz-bike-helmet": "A little girl with a black ponytail riding a purple bicycle on a park path, clearly wearing a bright red safety helmet strapped under her chin, green trees and a plain blue sky with white clouds behind her, no sun visible in the sky. No letters, no words, no numbers, no writing anywhere, no faces on any object in the sky.",
