...
export default function KInteractiveAuditPage() {
  ...
  function markResult(qId: string, result: TestResult) {
    if (result === "untested") {
      ...
      upsertReview(qId, { status: result, comment: current?.comment || "", grade: "kindergarten" });
    }
  }
  ...
  function saveComment(qId: string) {
    ...
    if (!text) return;
    const current = myReviews[qId];
    upsertReview(qId, { status: current?.status as any || null, comment: text, grade: "kindergarten" });
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-1">
        <h1 className="text-2xl font-bold">Interactive Question Audit</h1>
        <button onClick={() => exportCsv("k-interactive-audit.csv")}
