...
export default function AssessmentNameTurn({ childId, name, ready, onDone }: { childId: string; name: string; ready: boolean; onDone: () => void }) {
  ...
  return <div className="pa-intro pa-name-turn">
    ...
    {ready && <SayNameControl mode="child" autoStart writtenName={name} value={saidAs} onChange={setSaidAs} />}
    <button className="pa-primary" disabled={saving || !ready} onClick={() => void finish()}>{saving ? "Saving…" : saidAs ? "Let’s read" : "Skip for now"}</button>
