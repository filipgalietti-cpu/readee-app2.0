...
export default function JourneyAdventure({
  ...
  useEffect(() => {
    ...
    try { if (sessionStorage.getItem(openingKey)) setPhase('ready'); } catch { /* Optional UI storage. */ }
  }, [fromReport, openingKey]);
  useEffect(() => {
    if (actualPhase !== 'building') return;
    const timer = setTimeout(() => {
      setPhase('ready');
      try { sessionStorage.setItem(openingKey, 'seen'); } catch { /* Optional UI storage. */ }
    }, 3200);
  ...
  async function collect(id: string, amount: number) {
    ...
    try { await onReward(id, amount); } catch { setRewardError(true); }
  ...
  return (
    <div className={`${styles.experience} ${styles.liveExperience}`} data-journey data-live-journey data-reduced={reduced}>
      <header className={styles.header}>
        <Link href="/dashboard" className={styles.wordmark} aria-label="Readee dashboard">read<span>ee</span><i>®</i></Link>
        <div className={styles.readerHeading}>
          <h1>{model.reader.name}’s reading journey</h1>
          <span>{model.completed.size} of {model.lessons.length} lessons complete</span>
        </div>
        <div className={styles.headerTools}>
          <button className={styles.soundControl} onClick={() => setSoundOn((value) => !value)} aria-label={soundOn ? 'Mute journey sounds' : 'Enable journey sounds'} aria-pressed={soundOn}><Glyph name={soundOn ? 'volume2' : 'volume-x'} size={20} /></button>
          <button onClick={onPlan} aria-label="Why this journey?"><Glyph name="book-open" size={19} /> Why this journey?</button>
          <details className={styles.reviewMenu}>
            <summary aria-label="Browse reading journey"><Glyph name="map" size={20} /></summary>
            <div className={styles.reviewPopover}>
              <label className={styles.journeyBrowseLabel}>Explore your path
                <select aria-label="Choose journey chapter" value={chapterIndex} onChange={(event) => selectChapter(Number(event.target.value))}>
                  {model.chapters.map((entry, index) => <option key={entry.id} value={index}>{entry.grade} · {entry.name} · {entry.part}/{entry.parts}</option>)}
                </select>
              </label>
      ...
      <div className={styles.main}>
        {chapter ? <section className={styles.adventure} aria-label={`${model.reader.name}’s adventure`}>
          <JourneyWorld key={chapter.id} fixture={model.reader} chapter={chapter}
            completed={displayCompleted} checkpointComplete={chapterDone} milestoneComplete={rewardClaimed}
            bunnyPresent={bunnyPresent} phase={actualPhase === 'magic' ? 'insights' : actualPhase}
            intro={actualPhase === 'magic' ? <JourneyMagicReveal soundEnabled={soundOn} onComplete={showMap} /> : undefined}
            reduced={reduced} travel={travel} onArrival={arrival}
            onLesson={(lesson) => onStart(lesson.lessonId)} onAssessment={onPlan}
            onCheckpoint={() => setReview(true)} onMilestone={() => setReview(true)} onInterrupt={interrupt} />
        </section> : <section className={styles.emptyJourney}><h2>Let’s find a starting point.</h2><Link href={`/assessment?child=${encodeURIComponent(childId)}`}>Start the reading assessment</Link></section>}
      </div>
      <p className={styles.srOnly} role="status">{travel ? 'Your bunny is moving to the next destination.' : `${model.completed.size} lessons complete.`}</p>
      {review && chapter && <JourneyPlanDialog onClose={() => setReview(false)}>
        <div className={styles.liveReview}>
          ...
          <p>{chapter.lessonIds.filter((id) => model.completed.has(id)).length} of {chapter.lessonIds.length} lessons complete in this part.</p>
          <p>{chapter.reason}</p>
          <ul>{chapter.lessonIds.map((id) => <li key={id}><Glyph name={model.completed.has(id) ? 'check' : 'book-open'} size={18} /><button onClick={() => { setReview(false); onStart(id); }}>{model.lessons.find((lesson) => lesson.lessonId === id)!.title}</button></li>)}</ul>
          {chapter.rewardId && unitDone && !rewardClaimed && <button className={styles.primary} disabled={rewardBusy} onClick={() => collect(chapter.rewardId!, 20)}>{rewardBusy ? 'Saving…' : 'Collect your unit keepsake'} <FluentIcon name="carrot" size={20} /> 20</button>}
          {rewardClaimed && <p>Your unit keepsake is collected.</p>}
          {allDone && !openedChests.includes('__trophy__') && <button className={styles.primary} disabled={rewardBusy} onClick={() => collect('__trophy__', 50)}>Collect your journey trophy <FluentIcon name="carrot" size={20} /> 50</button>}
          {rewardError && <p role="alert">We couldn’t save the keepsake. Please try again.</p>}
          {chapterIndex < model.chapters.length - 1 && <button className={styles.primary} onClick={next}>{chapterDone ? 'Explore the next part' : 'Look ahead'} <Glyph name="arrow-right" size={18} /></button>}
        </div>
      </JourneyPlanDialog>}
    </div>
  );
