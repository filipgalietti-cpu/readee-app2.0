...
export default function ShareWithCommunityToggle({
  ...
  return (
    <div className="relative">
      ...
      {flash && (
        <div className="absolute left-0 top-full z-30 mt-2 w-72 rounded-xl border border-zinc-200 bg-white p-3 shadow-lg ring-1 ring-zinc-100">
          <button
            type="button"
            onClick={() => setFlash(null)}
            className="absolute right-1.5 top-1.5 rounded-full p-0.5 text-zinc-400 hover:bg-zinc-100"
            aria-label="Dismiss"
          >
            <X className="h-3 w-3" />
          </button>
      ...
      {err && (
        <div className="absolute left-0 top-full z-30 mt-2 w-72 rounded-xl border border-red-200 bg-red-50 p-3 text-xs text-red-800 shadow-lg">
          {err}
        </div>
      )}
    </div>
