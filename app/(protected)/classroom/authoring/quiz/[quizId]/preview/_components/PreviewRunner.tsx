...
import { Eye, Play, Volume2, Check, Lightbulb, Sparkles, BookOpen } from "lucide-react";
...
function CardsPreview({
  ...
  // Mirror the student-side flow: lead with a passage card (image,
  // body, audio) that fills the same slot the kid sees on the intro
  // screen. Then the question cards suppress media that matches the
  // hero so the carousel doesn't repeat the same image and audio
  // controller on every slide.
  const hasPassage = !!quizDescription || !!passageImage || !!passageAudio;
  const totalSlides = (hasPassage ? 1 : 0) + questions.length;
    ...
    <div className="px-2">
      <Carousel className="w-full">
        <CarouselContent>
            ...
            <CarouselItem className="md:basis-3/4 lg:basis-2/3">
              <Card className="overflow-hidden">
                <CardHeader className="!p-5 !pb-3">
                  ...
                  <CardTitle className="!text-base !mt-2">{quizTitle}</CardTitle>
                </CardHeader>
                <CardContent className="!p-5 !pt-0 space-y-3">
                  ...
                  {quizDescription && (
                    ...
                    >
                      <div className="whitespace-pre-line">{quizDescription}</div>
                    </article>
                  )}
                  {passageAudio && (
          ...
          {questions.map((q, i) => {
            const showImage = q.imageUrl && q.imageUrl !== passageImage;
            const showAudio = q.audioUrl && q.audioUrl !== passageAudio;
            return (
