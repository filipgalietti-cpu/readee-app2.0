...
> {
  ...
  try {
    ...
    if (provider === "vertex") {
      ...
      // Lazy-import the server-only helper so client bundles never
      // walk the google-auth-library import graph (it pulls in
      // child_process / fs which aren't browser-safe).
      const { generateSpeechVertex, VERTEX_TTS_MODEL } = await import("./vertex-tts");
