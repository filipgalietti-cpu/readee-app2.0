...
export async function loadOwnedPlacementPlan(childId: string): Promise<PlacementPlan | null> {
  ...
  if (!user) return null;
  const child = await db.from("children").select("id").eq("id", childId).eq("parent_id", user.id).maybeSingle();
  if (child.error) throw new Error("Could not load the reader. Please retry.");
  if (!child.data) return null;
  const saved = await db.from("placements").select("plan, decision, moments, created_at")
  ...
  return plan.version === 2 ? plan : buildPlan({
